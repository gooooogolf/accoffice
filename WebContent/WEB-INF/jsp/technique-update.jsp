...
<title>accoffice: Technique</title>
...
<jsp:include page="nav.jsp"></jsp:include>
        ...
        <div id="page-wrapper">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-lg-12">
                        <h1 class="page-header">UPDATE เทคนิคการใช้งาน</h1>
                    </div>
                    <!-- /.col-lg-12 -->
                </div>
                <!-- /.row -->
	            <div class="row">
	                <div class="col-lg-12">
	                    <div class="panel panel-default">
	                        <div class="panel-heading">เทคนิคการใช้งาน</div>
	                        <div class="panel-body">
	                            <div class="row">
	                            <form role="form" action="javascript:void(0)" method="post" id="techForm">
	                                <input type="hidden" id="id" name="id" value="${tech.id }">
	                                <div class="col-lg-6"> 
	                                		...
	                                		<div class="form-group">
	                                            <label>ข้อ</label>
	                                            <input class="form-control" id="techSeq" name="techSeq" value="${tech.techSeq }">
	                                        </div> 
	                                        <div class="form-group">
	                                            <label>เทคนิค</label>
	                                            <input class="form-control" id="techName" name="techName" value="${tech.techName }">
	                                        </div>	
	                                     	<div class="form-group">
	                                            <label>ข้อความเทคนิค</label>
	                                            <textarea class="form-control" rows="5" id="techTitle" name="techTitle">${tech.techTitle }</textarea>
	                                        </div>	                                        	                                    	                                         	                                     
	                               	</div>
	                               	<div class="col-lg-6">
	                               		<div class="form-group">
                                            <label>รายละเอียด</label>
                                            <textarea class="form-control" rows="15" id="techDesc" name="techDesc">${tech.techDesc }</textarea>
...
$(document).ready(function() {  
	...
	var productId = '${tech.productId }'
	if (productId.length != 0) $('#productId').val('${tech.productId }');
    ...
    $('#submit').click(function(){
    	var formData = $('#techForm').serializeObject();
...
function save(formData) {
	$.ajax({
	    url: '${pageContext.request.contextPath}/tech/update.do',
	    data: formData,
	    type: "POST",
	    dataType:"json",
	    ...
	    success: function(ret) {
	    	...
	    	$('.panel-heading').text('Technique Update!!!');
			window.location = '${pageContext.request.contextPath}/tech/list';
	    },
...
function validate(formData) {
	if (formData.techName == '') {
		$('.panel-heading').text('กรุณากรอกเทคนิคการใช้งาน');
		window.scroll(0, 0); 
		$('#techName').select();
		return false;	
	}
